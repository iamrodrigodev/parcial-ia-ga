...
import { ejecutarSeleccionRanking } from "@/features/geneticos/lib/seleccion";
...
export function Ranking() {
  ...
  const rankData = useMemo(() => ejecutarSeleccionRanking(individuos, 0).individuosRankeados, []);
    ...
    <Tarjeta>
      <EncabezadoTarjeta>
        ...
        <DescripcionTarjeta className="text-base mt-2">
          Evita que los individuos superdotados acaparen toda la ruleta. En lugar de usar el fitness absoluto, los individuos se ordenan y las porciones se asignan según su posición.
        </DescripcionTarjeta>
      </EncabezadoTarjeta>
      <ContenidoTarjeta>
                ...
                <div className="flex flex-col gap-2 text-xs font-medium">
                  {rankData.map((ind) => (
                    <div key={ind.id} className={`flex items-center justify-between p-1.5 rounded transition-all ${selectedId === ind.id ? "bg-blue-50 ring-1 ring-blue-500 shadow-sm transform scale-105" : ""}`}>
                      ...
                      <span className={selectedId === ind.id ? "font-bold text-blue-700" : "text-muted-foreground"}>{ind.prob.toFixed(1)}%</span>
                    </div>
                  )).reverse()}
                </div>
              </div>
